...
document.addEventListener('DOMContentLoaded', () => {
    const bubble = document.getElementById('custom-status-bubble');
    const textEl = document.getElementById('custom-status-text');
    const emojiEl = document.getElementById('custom-status-emoji');

    if (!bubble || !textEl || !emojiEl) return;

    // Khôi phục trạng thái đã lưu
    const savedText = localStorage.getItem('customStatusText');
    const savedEmoji = localStorage.getItem('customStatusEmoji');

    if (savedText) textEl.textContent = savedText;
    if (savedEmoji) emojiEl.textContent = savedEmoji;

    updateBubbleState();

    bubble.addEventListener('click', () => {
        const newEmoji = prompt('Nhập emoji trạng thái:', emojiEl.textContent || '💻');
        if (newEmoji === null) return;

        const newText = prompt('Nhập nội dung trạng thái:', textEl.textContent || '');
        ...
        localStorage.setItem('customStatusEmoji', emojiEl.textContent);
        localStorage.setItem('customStatusText', textEl.textContent);
...
document.getElementById('copy-email').addEventListener('click', function() {
    ...
    navigator.clipboard.writeText(email).then(() => {
        // Thay đổi nội dung nút tạm thời để thông báo
        const span = this.querySelector('span');
        const originalText = span.innerText;
        
        span.innerText = "Copied!";
        this.classList.add('copied'); // Bạn có thể thêm CSS cho class này nếu muốn
